...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  useEffect(() => {
    if (range.from && range.to) {
      // Create the updated query string using the createQueryString function
      const queryString = new URLSearchParams(searchParams.toString());

      queryString.set('startDate', range.from.toISOString().split('T')[0]);
      queryString.set('endDate', range.to.toISOString().split('T')[0]);

      // Update the URL without duplicating params
